...
(function($, window) {
  ...
  var Mi18n = {
     ...
     * @return null;
     **/
    addQueue: function(sourceLanguage) {
      var _this = this;
      ...
      for (var i in sourceLanguage) {
        if (_this.searchLanguageValueReturnKey(sourceLanguage[i]) == null) {
          if (_this.tempData.addedLanguage.indexOf(sourceLanguage[i]) == -1) {
            _this.tempData.addLanguage.push(sourceLanguage[i]);
          }
        }
      }
      this.add();
    ...
    add: function() {
      var _this = this;
      if (_this.tempData.addLanguage.length > 0) {
        $.ajax({
          type: 'POST',
          async: true,
          dataType: "json",
          ...
          data: {
            ...
            language: JSON.stringify(_this.tempData.addLanguage),
            page: window.location.pathname
          },
          success: function(result) {
            if (result == 'true'){
              _this.tempData.addedLanguage.push(_this.tempData.addLanguage);
            } else {
              _this.tempData.addLanguage = [];
            }
          },
          error: function() {
            setTimeout(function() {
    ...
    gather: function(el) {
      ...
      $('.mi18n', el).each(function(i) {
        ...
        if ($(this).attr('label') && $(this).attr('label') != '') {
          if (_this.language[_this.md5($(this).attr('label'))] == undefined) {
            sourceLanguage.push($(this).attr('label'));
          }
        }
    ...
    get: function() {
      ...
      if (this.cache.get('cacheMi18nExpires') && Date.parse(new Date()) < this.cache.get('cacheMi18nExpires')){
        this.language = JSON.parse(this.cache.get('cacheMi18nLanguage'));
      } else {
        var result = $.ajax({
          url: _this.config.getLanguageUrl,
          data: {
            'page': _this.config.page,
            'website': _this.config.website
          },
          async: false
        }).responseText;
        this.language = JSON.parse(result);
        this.cache.set('cacheMi18nLanguage', result);
        this.cache.set('cacheMi18nExpires', Date.parse(new Date())+this.config.cacheTime*1000);
      }
    },
    ...
    replaceLanguage: function(el) {
      ...
      if (el.html() != '') {
        ...
        var mi18nText;
        if (el.attr('data-set-mi18n-language')){
          var language = JSON.parse(el.attr('data-set-mi18n-language'));
          if (language[this.websiteLanguage]){
            mi18nText = language[this.websiteLanguage];
          } else {
            if (index = this.searchLanguageValueReturnKey(transformHTML.html)) {
              mi18nText = this.language[index][this.websiteLanguage];
            }
          }
        } else {
          if (index = this.searchLanguageValueReturnKey(transformHTML.html)) {
        ...
        mi18nText && el.html(mi18nText);
        if (mi18nVar.length > 0 || mi18nVarSourceHtml.length > 0) {
          $('.mi18n-var', el).each(function(i) {
            if ($(this).attr('data-set-mi18n-stop-translate') != 1) {
              mi18nVar[i] && $(this).html(mi18nVar[i]);
            } else {
              mi18nVarSourceHtml[i] && $(this).html(mi18nVarSourceHtml[i]);
            }
          });
        }
      }
      ...
      if (el.attr('data-set-mi18n-src')) {
        ...
        if (el.attr('data-set-mi18n-translate-background') == 1) {
          src[this.websiteLanguage] && el.css({
            "backgroundImage": "url('" + src[this.websiteLanguage] + "')"
          });
        }
        src[this.websiteLanguage] && el.attr('src', src[this.websiteLanguage]);
      }

      if (el.attr('data-set-mi18n-href')) {
        var href = JSON.parse(el.attr('data-set-mi18n-href'));
        href[this.websiteLanguage] && el.attr('href', href[this.websiteLanguage]);
      }
          ...
          if (el.attr('data-set-mi18n-language')){
            ...
            if (language[this.websiteLanguage]){
              ...
              if (index = this.searchLanguageValueReturnKey(el.val())) {
                this.language[index][this.websiteLanguage] && el.val(this.language[index][this.websiteLanguage]);
              }
            }
          } else {
            if (index = this.searchLanguageValueReturnKey(el.val())) {
        ...
        if (el.attr('data-set-mi18n-language')){
          ...
          if (language[this.websiteLanguage]){
            ...
            if (index = this.searchLanguageValueReturnKey(el.attr('title'))) {
              this.language[index][this.websiteLanguage] && el.attr('title', this.language[index][this.websiteLanguage]);
            }
          }
        } else {
          if (index = this.searchLanguageValueReturnKey(el.attr('title'))) {
        ...
        if (el.attr('data-set-mi18n-language')){
          ...
          if (language[this.websiteLanguage]){
            ...
            if (index = this.searchLanguageValueReturnKey(el.attr('placeholder'))) {
              this.language[index][this.websiteLanguage] && el.attr('placeholder', this.language[index][this.websiteLanguage]);
            }
          }
        } else {
          if (index = this.searchLanguageValueReturnKey(el.attr('placeholder'))) {
      ...
      if (el.attr('label') && el.attr('label') != '') {
        if (el.attr('data-set-mi18n-language')){
          var language = JSON.parse(el.attr('data-set-mi18n-language'));
          if (language[this.websiteLanguage]){
            mi18nText = language[this.websiteLanguage];
          } else {
            if (index = this.searchLanguageValueReturnKey(el.attr('label'))) {
              this.language[index][this.websiteLanguage] && el.attr('label', this.language[index][this.websiteLanguage]);
            }
          }
        } else {
          if (index = this.searchLanguageValueReturnKey(el.attr('label'))) {
        ...
        if (el.attr('data-set-mi18n-language')){
          ...
          if (language[this.websiteLanguage]){
            ...
            if (index = this.searchLanguageValueReturnKey(el.attr('alt'))) {
              this.language[index][this.websiteLanguage] && el.attr('alt', this.language[index][this.websiteLanguage]);
            }
          }
        } else {
          if (index = this.searchLanguageValueReturnKey(el.attr('alt'))) {
      ...
      if (el.attr('data-set-mi18n-class')){
          var className = JSON.parse(el.attr('data-set-mi18n-class'));
          for(var i in className){
            if (el.hasClass(className[i])){
              className[i] && el.removeClass(className[i]);
            }
            className[this.websiteLanguage] && el.addClass(className[this.websiteLanguage]);
          }
      }
    ...
    transformHTML: function(html) {
      var sourceHtml = html.html(),
        tempHtml = html.html();
      if ($('.mi18n-var', '<language>'+sourceHtml+'</language>').length > 0) {
        tempHtml = $('.mi18n-var', '<language>'+tempHtml+'</language>').empty();
        tempHtml = tempHtml.prevObject[0].innerHTML;
      }
      var data = {
    ...
    */
    ...
    bindMi18nsEvent: function(el) {
      var _this = this;
      if (el.hasClass('mi18ns')) {
        // 侦听区域更新，该区域<div class="mi18ns"></div>会动态更新 HTML，增加 <ins class="mi18n">文本</ins>
    ...
    translateText: function(text) {
      ...
      if (this.language) {
        ...
        var language = [];
        language[_this.websiteLanguage] = text;
        return this.language[index] ? this.language[index] : language;
      } else {
        return text;
      }
    },
    ...
    init: function() {
      ...
      if (this.config.languages.indexOf(browserLanguage) == '-1'){
        browserLanguage = this.config.defaultLanguage;
        this.cookie.set('systemLanguage', browserLanguage, '/');
      }
